...
import org.librairy.solr.model.Stats;
...
//            if (counter.getAndIncrement() == 1000) break;
        ...
        sample.parallelStream().forEach( d1 -> {
            List<String> simDocs = documents.stream().map(d2 -> new Score(JSDSimilarity.btw(d1.getShape(), d2.getShape()), d1, d2)).sorted((a, b) -> -a.getValue().compareTo(b.getValue())).limit(numSimilarDocs).map(s -> s.getSimilar().getId()).collect(Collectors.toList());
    ...
    public void newFrom(RepresentationalAlgorithm algorithm) throws IOException {
        LOG.info("creating a base-directory for index");
        File indexFile = new File("target/index-" + algorithm.id());
        ...
        while ((line = bufferedReader.readLine()) != null) {
            ...
            String stringTopics = algorithm.representationOf(shape);
